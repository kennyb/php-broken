...
PHP_FUNCTION(chdir)
{
	...
	if ((SAFE_MODE && !php_checkuid(str, NULL, CHECKUID_CHECK_FILE_AND_DIR)) || php_check_open_basedir(str TSRMLS_CC)) {
		RETURN_FALSE;
	}
	ret = VCWD_CHDIR(str);
...
no_results:
	...
	for (n = 0; n < globbuf.gl_pathc; n++) {
		if (SAFE_MODE || (PG(open_basedir) && *PG(open_basedir))) {
			if (SAFE_MODE && (!php_checkuid_ex(globbuf.gl_pathv[n], NULL, CHECKUID_CHECK_FILE_AND_DIR, CHECKUID_NO_ERRORS))) {
				basedir_limit = 1;
				continue;
